...
- **D13 — Cadence de contenu élargie : viser 3 posts/jour** (la fréquence fait grossir un compte jeune). Recap du matin + **post preview séparé** chaque soir (format adaptable : 1 gros match en 3 slides + 1 slide par autre match du jour ; gérer les journées à plusieurs petits matchs) + 3e slot. → soit **+2 carrousels/jour** à industrialiser (dépend du chantier A pour rester tenable en solo).
  - ✅ **3e slot résolu (14/06)** : moteur piloté par le calendrier (`npm run day`) — quiz tiré des faits les jours de match, état du tournoi (`/standings`) les jours creux, evergreen IA en secours (usine `npm run evergreen` + barrière `[VERIFY]`/`--verify`). Marche jusqu'à la finale. Recap + preview restent à relire/éditer (l'humain choisit l'angle).
