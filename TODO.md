...
- [x] **[CLAUDE] Refonte du post preview en format TIERED (15/06)** ✅ — `preview.json` repensé : slide **OVERVIEW** (tous les matchs du jour d'un coup d'œil) → gros match(s) en **2 slides** (*Otto's Board* : prono **1/X/2** entouré + le call d'Otto + **key battle** « X → Y » à la flèche dessinée ; *Numa's Rewind* : **un chiffre** + head-to-head + 1 anecdote marquante) → **cartes compactes** pour le reste → cta. **Cast dessiné sur chaque slide** (orange→Otto, bleu→Numa ; *Vera's File* discipline en situationnel, rouge). Systématisé partout : `templates/slides/preview.js` + `slide.css`, scaffold `fetch.mjs --preview`, `content/_schema.md`, message `day.mjs`. 1er cas réel : `content/2026-06-15/` (4 matchs, 2 gros — Belgium·Egypt + Saudi·Uruguay).
- [ ] **[ENSEMBLE] Robustesse preview + impact stories** — voir **`NEXT_STEPS.md`** (15/06) : (1) 3+ gros matchs auto dans le scaffold ; (2) overview **adaptatif** pour les journées à 6 matchs (déborde aujourd'hui) ; (3) générer des **stories avant-match** depuis `preview.json` (aucun lien actuellement). Non bloquant pour la cadence actuelle.
